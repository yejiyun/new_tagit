$(function() {
	...
	function asideGet() {
		...
		aside.find(".footer .btn_apply").click(
				function() {
					var thumbnail = aside.find(".article .thumbnail .image")
							.attr("data-thumbnail"), tags = [];
					aside.find(".tokenizer li span.label").each(function() {
						tags.push($(this).text());
					});
					var data = {
						attachment : aside.find(".article").attr("data-id"),
						url : aside.find(".article .thumbnail .image").attr(
								"data-url"),
						content : aside.find(".article .content").text(),
						thumbnail : thumbnail,
						memo : aside.find(".article .memo .body .txt_memo")
								.val(),
						tags : tags.join(",")
					}
					if(tags.length <= 0) {
					...
					$.ajax({
						...
						success : function(response) {
							if (response.state) {
								...
								aside.find(".tokenizer li span.label").each(
										function() {
											$(this).parent().remove();
										});
		...
		var setFeed = function(feed) {
			...
			if (feed.data.length) {
				...
				aside.find(".article .thumbnail .image").attr("data-thumbnail",
						data.picture);
				aside.find(".article .thumbnail .image").css(
						"background-image", "url(" + data.picture + ")");
				aside.find(".article .thumbnail .image").off("click");
				if (data.link) {
					aside.find(".article .thumbnail .image").attr("data-url",
							data.link);
					aside.find(".article .thumbnail .image").on("click",
							function() {
				...
				if (feed.paging) {
					...
					aside.find(".article .thumbnail .prev").off("click").on(
							"click", function() {
								getPaging(feed.paging.previous);
							});
					aside.find(".article .thumbnail .next").off("click").on(
							"click", function() {
								getPaging(feed.paging.next);
							});
				}
				...
				.ajax({
					...
					success : function(response) {
						if (response.state) {
							...
							detail.find(".article .image img").attr("src",
									response.data.thumbnail);
							detail.find(".article .content .wrap .wrap").text(
		...
		$.ajax({
			...
			success : function(response) {
				if (response.state) {
					...
					aside.find(".article").attr("data-id", data.id);
					aside.find(".article .memo .body .txt_memo")
							.text(data.memo);
					aside.find(".article .content").text(data.content);
					aside.find(".article .thumbnail .image").attr(
	...
	$(document).on("click", "aside#detail_edit .footer .btn_apply", function() {
		...
		$('.tokenizer li span.label').each(function() {
			tags.push($(this).text());
		});
		var data = {
			memo : memo,
